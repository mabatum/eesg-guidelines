...
HINTS = {
    ...
    "bone-sarcomas/osteosarcoma-adults": "AP, патоморфоз, операция и лечение рецидива",
    "bone-sarcomas/chondrosarcoma-adults": "ACT, обычная, дедифференцированная, мезенхимальная и светлоклеточная формы",
    "bone-sarcomas/ewing-sarcoma-adults": "VAC/IE, лучевая терапия, лёгочные метастазы и повторные линии",
    ...
    "bone-sarcomas/rare-bone-tumours": "классификация редких подтипов и специализированная помощь",
...
def homepage_markdown(publish: list[str]) -> str:
    ...
    lines.extend(
        [
            "## Источники редакции",
            "",
            "Основа раздела — клинические рекомендации «Саркомы костей», 2025, ID 532_5. "
            "[Полный документ (PDF)](https://mabatum.github.io/eesg-guidelines/_assets/sources/kr532-5-2025.pdf). "
            "Рядом с клиническими положениями указаны разделы и страницы источника. "
            "Веб-редакция предназначена для профессионального обсуждения.",
            "",
            "## Экспертное обсуждение",
            "",
            "Форма замечаний временно недоступна: выполняется подключение. "
            "После активации можно будет оставить замечание к странице или выделенному фрагменту.",
            "",
            "Для отправки замечаний не потребуется вход в GitHub. "
            "Ответы будут доступны только редактору.",
        ]
    )
