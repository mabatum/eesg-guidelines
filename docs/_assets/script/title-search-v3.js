(() => {
  ...
  function detectSiteRoot() {
    ...
    const ownScript = scripts.find((script) =>
      /\/_assets\/script\/title-search(?:-v\d+)?\.js(?:[?#]|$)/.test(script.src || ''),
    );
    if (ownScript?.src) return new URL('../../', ownScript.src);

    const anyAssetScript = scripts.find((script) => (script.src || '').includes('/_assets/script/'));
  ...
  const prepared = INDEX.map((item) => ({
    ...
    aliasNorms: (item.aliases || []).map(normalize).filter(Boolean),
  }));
  ...
  function score(item, query) {
    ...
    const exactAlias = item.aliasNorms.some((alias) => alias === query);
    const titleHasAll = terms.every((term) => item.titleNorm.includes(term));
    const aliasHasAll = item.aliasNorms.some((alias) => terms.every((term) => alias.includes(term)));
    const inAll = terms.every((term) =>
      item.titleNorm.includes(term)
      || item.contextNorm.includes(term)
      || item.aliasNorms.some((alias) => alias.includes(term)),
    );
    if (!inAll) return 0;

    let value = 100;
    if (item.titleNorm === query) value += 1100;
    else if (exactAlias) value += 950;
    else if (item.titleNorm.startsWith(query)) value += 700;
    else if (item.titleNorm.includes(query)) value += 500;
    else if (titleHasAll) value += 350;
    else if (aliasHasAll) value += 320;

    for (const term of terms) {
      if (item.titleNorm.startsWith(term)) value += 80;
      else if (item.titleNorm.includes(term)) value += 50;
      else if (item.aliasNorms.some((alias) => alias === term)) value += 70;
      else if (item.aliasNorms.some((alias) => alias.includes(term))) value += 40;
  ...
  function resultUrl(item) {
    const relative = String(item.url || '').replace(/^\.\//, '').replace(/^\//, '');
  ...
  function ensureDialog() {
      ...
      <section class="eesg-search-panel" role="dialog" aria-modal="true" aria-labelledby="eesg-search-title">
        <div class="eesg-search-head">
          <div>
            ...
            <p>Поиск по названиям страниц, разделов и медицинским сокращениям.</p>
          </div>
    ...
    root.querySelectorAll('[data-search-close]').forEach((element) => element.addEventListener('click', closeSearch));
    root.querySelector('#eesg-search-input').addEventListener('input', (event) => renderResults(event.target.value));
    return root;
  }
  ...
  function renderResults(value) {
    ...
    if (!query) {
      ...
      results.innerHTML = '<div class="eesg-search-empty">Начните вводить заболевание, препарат, раздел или сокращение.</div>';
      return;
    }

    const matches = search(query);
    meta.textContent = matches.length ? `Найдено: ${matches.length}${matches.length === 12 ? '+' : ''}` : 'Совпадений нет';
    if (!matches.length) {
      results.innerHTML = '<div class="eesg-search-empty">Попробуйте другое название или медицинское сокращение.</div>';
      return;
    }
